"""Advanced portfolio valuation and scenario engines.

This module models multi-asset product portfolios with rich cash-flow
construction, rNPV/DCF valuation, VC-style back-solving, scenario stress
testing, and optional time-series forecasting bridges.
"""
...
class ModelConfig:
    ...
    sales_ramp_factors: List[float] | None = None  # e.g. [0.2, 0.6, 1.0, 1.0, 1.0]
...
class Product:
    """Represents a single product/asset and builds its cash-flow table."""
    ...
    def build_cashflow_table(self) -> pd.DataFrame:
        """Build full P&L + FCFF (before WC) for this product."""
...
class Portfolio:
    ...
    def consolidated_table(self) -> Dict[str, pd.DataFrame | pd.Series]:
        """Return per-product detail and consolidated FCFF after working capital."""
...
class ValuationEngine:
    ...
    def _add_terminal_value(self, dcf_df: pd.DataFrame, cons_df: pd.DataFrame) -> float:
        ...
        rnpv = dcf_df["discounted_fcff"].sum() + dcf_df.get("discounted_terminal_value", pd.Series()).sum()
        return float(rnpv)
...
class ScenarioEngine:
    """Applies systematic shocks and re-runs the valuation."""
    ...
    def _apply_scenario(self, scenario: Scenario) -> Portfolio:
        ...
        for prod in self.base_portfolio.products:
            ...
            cfg_dict["success_prob"] = max(0.0, min(1.0, cfg_dict["success_prob"] * scenario.success_prob_multiplier))
...
class MonteCarloEngine:
    """Monte Carlo engine that perturbs revenue and cost drivers."""
...
class ForecastEngine:
    """
    Wraps ARIMA / Prophet / LSTM time-series models and translates their forecasts
    into adjustments on ProductConfig. Heavy dependencies are imported lazily.
    """
    ...
    def forecast_prophet(
        ...
        """Prophet forecast for a DataFrame with columns 'ds' and 'y'."""
    ...
    def forecast_lstm(
        ...
        """Very lightweight LSTM forecaster (optional TensorFlow dependency)."""
    ...
    def apply_price_forecast_to_products(
        ...
        """Use a price forecast to adjust product revenue trajectories."""
...
# ===========================================================
# 8. Forecast → Scenario bridge (Prophet-driven stress tests)
# ===========================================================


class ForecastScenarioBridge:
    """Bridges ForecastEngine outputs into ScenarioEngine stress tests."""
    ...
    def build_price_scenarios_from_prophet(
        ...
        scen_results = scen_engine.run_scenarios(scenarios=scenarios, ebitda_year_offset=ebitda_year_offset)
        return scen_results
...
def demo_portfolio() -> ValuationResult:
...
if __name__ == "__main__":  # pragma: no cover
    ...
    print("rNPV:", round(result.rnpv, 2), result.portfolio.model_config.currency)
