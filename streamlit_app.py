...
st.markdown("### 1) Configure the automobile and assembly line")
with st.form("plan_form", border=True):
    ...
    if submitted:
        try:
            ...
            st.success("Capacity calculated. Scroll down to view results.")
        except Exception as exc:  # pragma: no cover - UI feedback
            st.session_state.plan_result = None
            st.error(f"Unable to calculate capacity: {exc}")

st.markdown("### 2) Review capacity and component requirements")
if not st.session_state.plan_result:
    st.info("Submit the form above to see throughput and requirements.")
else:
...
st.markdown("### 3) Optional: RAG feasibility service")
st.write(
